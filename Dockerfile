...
RUN useradd -ms /bin/bash unreal && echo "unreal:unreal" | chpasswd \
	&& adduser unreal sudo \
	&& echo "unreal ALL=(ALL) NOPASSWD: ALL" >> /etc/sudoers
USER unreal

RUN cd ~/ && git clone https://github.com/Microsoft/AirSim.git && git checkout f78f11774055fa7381c9cdf6dc08265c93864226 && cd AirSim/cmake && sudo bash ./getlibcxx.sh; exit 0
...
COPY UnrealEngine-4.15.0-release.zip /home/unreal/UnrealEngine-4.15.0-release.zip
RUN cd ~/ && unzip -q UnrealEngine-4.15.0-release.zip && rm UnrealEngine-4.15.0-release.zip
...
RUN cd ~/UnrealEngine-4.15.0-release && ./Setup.sh
RUN cd ~/UnrealEngine-4.15.0-release && ./GenerateProjectFiles.sh -project="/home/unreal/AirSim/Unreal/Environments/Blocks/Blocks.uproject" -game -engine
